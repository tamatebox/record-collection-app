...
// ジャンルフィルター関数
const genreFilter = (record, genres) => 
  genres.length === 0 || genres.includes(record.genre);

// 年代フィルター関数
const decadeFilter = (record, decades) => 
  decades.length === 0 || 
  decades.some(decade => 
    parseInt(record.release_year, 10) >= decade && 
    parseInt(record.release_year, 10) < decade + 10
  );
...
// サイズフィルター関数
const sizeFilter = (record, sizes) => 
  sizes.length === 0 || sizes.includes(record.size);
...
export const useRecords = () => {
  ...
  const handleUpdateRecord = async (id, updatedRecord) => {
    try {
      ...
      setRecords(prev => 
        prev.map(record => record.id === id ? result : record)
      );
